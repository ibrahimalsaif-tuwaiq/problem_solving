...
const count = (target, input) => {
  return input.reduce((acc, array) => {
    const foundArray = array.filter((number) => number === target);
    acc += foundArray.length;
    return acc;
  }, 0);
};
...
const totalSum = (input) => {
  return input.reduce((acc, array) => {
    const arraySum = array.reduce((acc, number) => {
      acc += number;
      return acc;
    }, 0);
    acc += arraySum;
...
let findShortest = (data) => {
  const shortestCharacter = data.reduce((acc, character) => {
    return Number(acc.height) < Number(character.height) ? acc : character;
  });
